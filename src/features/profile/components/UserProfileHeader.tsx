...
import { MessageCircle, MoreHorizontal, MapPin, Calendar, Edit3, Settings } from 'lucide-react';
...
export default function UserProfileHeader({ user, isMe }: UserProfileHeaderProps) {
    const tags = ['Design', 'UX', 'CreativeLife', 'TechEnthusiast'];
    const joinDate = user.created_at ? new Date(user.created_at).toLocaleDateString('en-US', { month: 'long', year: 'numeric' }) : user.joinDate;
            ...
            <div className="px-8 pb-8 -mt-20">
                <div className="flex justify-between items-end mb-6">
                    <div className="relative">
                        <Avatar
                            src={user.avatar_url || user.display_avatar_url || ''}
                    ...
                    <div className="flex gap-3 -mb-2 ">
                        {isMe ? (
                            <>
                                <button className="px-6 py-2.5 bg-neutral-100 hover:bg-neutral-200 text-neutral-800 rounded-xl font-bold transition-all shadow-sm flex items-center gap-2">
                            ...
                            <>
                                <button className={`px-8 py-2.5 rounded-xl font-bold transition-all shadow-lg flex items-center gap-2 ${user.is_following
                                    ? 'bg-neutral-200 text-neutral-800 hover:bg-neutral-300'
                                    : 'bg-primary-200 hover:bg-primary-300 text-neutral-100 shadow-primary-200/20'
                                    }`}>
                                    {user.is_following ? (
                                        <span>Following</span>
                                    ) : (
                ...
                <div className="space-y-4">
                    <div className="flex items-center gap-2">
                        <h1 className="text-3xl font-black text-neutral-800 tracking-tight">{user.full_name}</h1>
                        {user.is_verified && (
                    ...
                    <p className="text-neutral-600 font-medium">@{user.username}</p>
